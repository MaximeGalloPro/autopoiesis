...
Le moteur actuel est une grille bornée de `40 × 24`. Les coordonnées hors-bord
sont refusées, le déplacement reste discret (une case cardinale) et
`Simulation::execute` vérifie la praticabilité au moment où il applique
l’action. Le contrat de navigation ci-dessous prépare une évolution vers des
trajectoires continues sans modifier encore cette autorité.

Ce lot ne branche donc pas encore une navmesh au monde courant. Il crée le
...
L’étape suivante est un backend optionnel, épinglé à une révision
RecastNavigation, derrière `INavigation` :

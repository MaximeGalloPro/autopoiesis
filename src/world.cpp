...
bool World::hunt_rabbit(Position hunter) {
  if (!rabbit_alive_ || std::abs(hunter.x-rabbit_.x)+std::abs(hunter.y-rabbit_.y)!=1) return false;
  rabbit_alive_=false;
  return true;
}
void World::advance_nature(std::mt19937& rng) {
  if (!rabbit_alive_) return;
...
std::string World::ascii(const std::vector<Agent>& agents) const {
  ...
  for (int y=0;y<height;++y) { for (int x=0;x<width;++x) { Position p{x,y}; char c='.'; switch(terrain(p)){case Terrain::Wall:c='#';break;case Terrain::Water:c='~';break;case Terrain::Tree:c='T';break;case Terrain::Bush:c='B';break;default:break;} if (rabbit_alive_ && p==rabbit_) c='r'; for (const auto& a:agents) if (a.alive && a.position==p) c='A'; out<<c; } out<<'\n'; }
  return out.str();
}
}

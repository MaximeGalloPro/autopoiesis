...
int main(int argc,char** argv){
  int days=positive_int_from_env("SIMULATION_DAYS",100),delay=non_negative_int_from_env("SIMULATION_DELAY_MS",500),render_every_days=non_negative_int_from_env("SIMULATION_RENDER_EVERY_DAYS",1);unsigned seed=42;bool no_api=false;
  for(int i=1;i<argc;++i){std::string a=argv[i];auto next=[&](auto&v){if(i+1>=argc)throw std::runtime_error("missing argument for "+a);v=std::remove_cvref_t<decltype(v)>(std::stoll(argv[++i]));};if(a=="--days")next(days);else if(a=="--seed")next(seed);else if(a=="--delay-ms")next(delay);else if(a=="--render-every-days")next(render_every_days);else if(a=="--no-api")no_api=true;else{std::cerr<<"Unknown option: "<<a<<'\n';return 2;}}
  try{
...
#ifdef AUTOPOIESIS_GUI
    simulation.run(days,delay,render_every_days,validation_gate,graphical_interface.get());
#else

...
Decision LocalDecider::decide(const Perception& p) {
  ...
  const auto in_bounds=[&](Position position){return position.x>=0&&position.x<width&&position.y>=0&&position.y<height;};
  const auto step=[&](Position position,const std::string& direction)->std::optional<Position>{
    if(!in_bounds(position))return std::nullopt;
    if(direction=="north")--position.y;
    else if(direction=="south")++position.y;
    else if(direction=="east")++position.x;
    else if(direction=="west")--position.x;
    else return std::nullopt;
    return in_bounds(position)?std::optional<Position>{position}:std::nullopt;
  };
  ...
  const bool home_chest_full=[&]{
    if(!known_home)return false;
    const auto known_home_cell=known_cells.find({known_home->x,known_home->y});
    if(known_home_cell==known_cells.end())return false;
    const int capacity=known_home_cell->second.value("camp_chest_capacity",0);
    return capacity>0&&known_home_cell->second.value("camp_chest_occupation",0)>=capacity;
  }();
  ...
  if(!vital_emergency&&home_chest_full){
    const bool beside_home=known_home&&std::abs(self.x-known_home->x)+std::abs(self.y-known_home->y)==1;
    if(!beside_home)if(const auto fallback=return_to_known_home())return *fallback;
    if(has_action("upgrade_camp_chest"))
      return {DecisionType::Action,"upgrade_camp_chest",json::object(),
              "Le coffre est plein : j'augmente sa capacité.","foyer","","agrandir le coffre"};
    return {DecisionType::Action,"wait",json::object(),
            "Le coffre est plein et aucune transition locale ne le libère encore.","foyer","","attendre au camp"};
  }
  ...
  if(has_action("upgrade_camp_chest"))
    return {DecisionType::Action,"upgrade_camp_chest",json::object(),
            "J'agrandis le coffre du foyer avant de multiplier les réserves.","foyer","","agrandir le coffre"};
  ...
  if(best_goal=="social"){
    ...
    const auto map_shares=p.value.value("map_sharing_opportunities",json::array());
    if(has_action("share_map_knowledge")&&!map_shares.empty())return {DecisionType::Action,"share_map_knowledge",
        {{"target_id",map_shares.front().value("target_id","")}},
        "Je partage les cases cartographiées au foyer.","cartographie","","transmettre la carte connue"};
  ...
  if(repetition&&best_goal=="eat"&&hunger>=70){
    ...
    while(!pending.empty()){
      ...
      for(const auto& direction:directions){
        ...
        if(!next||reached.contains({next->x,next->y})||!traversable.contains({next->x,next->y})) continue;
    ...
    for(const auto& direction:directions){
      const auto next=step(self,direction);
      if(!next)continue;
  ...
  if(!me.value("equipped_tool",json(nullptr)).is_null()){
    ...
    for(const auto& building:p.value.value("buildings",json::array()))if(!building.value("complete",false)){
      ...
      for(const auto& direction:directions)if(const auto candidate=step(site,direction);candidate&&traversable.contains({candidate->x,candidate->y}))work_positions.insert({candidate->x,candidate->y});
    }
  ...
  if(carrying_food){
    ...
    for(const auto& coordinates:campfire_cells){
      ...
      for(const auto& direction:directions)if(const auto candidate=step(fire,direction);candidate&&traversable.contains({candidate->x,candidate->y}))camp_positions.insert({candidate->x,candidate->y});
    }
  ...
  if(best_goal=="hydrate"){
    ...
    for(const auto& coordinates:water_cells){Position water{coordinates.first,coordinates.second};for(const auto& direction:directions)if(const auto candidate=step(water,direction);candidate&&traversable.contains({candidate->x,candidate->y}))drink_targets.insert({candidate->x,candidate->y});}
  ...
  if(goal_state){
    ...
    for(const auto& coordinates:traversable){
      ...
      const bool borders_unknown=std::any_of(directions.begin(),directions.end(),[&](const std::string& direction){
        const auto next=step(candidate,direction);
        return next&&!known_position(*next);
      });
  ...
  for(const auto& direction:directions){
    const auto adjacent=step(self,direction);
...
static bool action_succeeded(const Decision& decision, const std::string& result) {
  ...
  if (decision.action == "upgrade_camp_chest") return result == "agrandit le coffre du camp";
  ...
  if (decision.action == "share_map_knowledge") return result.starts_with("transmet ");
...
static int action_duration_cycles(const Decision& decision) {
  ...
  if (decision.action == "craft_camp_item" || decision.action == "upgrade_camp_chest" || decision.action == "build_shelter" ||
  ...
  if (decision.action == "talk" || decision.action == "share_camp_meal" ||
      ...
      decision.action == "mourn" || decision.action == "teach_skill" ||
      decision.action == "share_map_knowledge") return 5;
  return 3;
}
...
static Agent initial_agent(std::string id,std::string name,Position position,int hunger,int fatigue,
  ...
  agent.project=std::move(project);agent.family_id=primary_family_id;
  return agent;
}
...
void Simulation::load_checkpoint() {
  ...
  if (!state.contains("group"))
    group_.migrate_completed_infrastructure(
        world_.has_completed_building(BuildingType::Stockpile),
        world_.has_completed_building(BuildingType::Workshop));
...
Perception Simulation::perceive(Agent& a) {
  ...
  if(!a.home_camp&&!a.known_campfires.empty()){
    ...
    std::sort(candidates.begin(),candidates.end(),[&](Position left,Position right){const int left_distance=world_.distance(a.position,left),right_distance=world_.distance(a.position,right);return left_distance!=right_distance?left_distance<right_distance:std::pair{left.x,left.y}<std::pair{right.x,right.y};});
  ...
  const auto map_shares=map_sharing_opportunities(a,world_,agents_);
  ...
  json buildings=json::array();for(const auto&[site,building]:world_.buildings())if(a.map_memory.contains({site.x,site.y})||world_.distance(a.position,site)<=3)buildings.push_back({{"x",site.x},{"y",site.y},{"type",building_type_name(building.type)},{"progress",building.progress},{"required_work",building.required_work},{"complete",building.complete},{"adjacent",world_.adjacent(a.position,site)}});
  json designations=json::array();if(const auto fire=world_.nearby_campfire(a.position)){
    ...
    for(const auto&[type,offset]:preferred)if(!world_.has_completed_building(type)){
      Position site{fire->x+offset.x,fire->y+offset.y};
      if(!world_.in_bounds(site))continue;
...
void Simulation::update_dangers() {
  auto warn=[&](DangerType type,Position position,int severity,int duration,
    ...
    if(!world_.in_bounds(position))return;
  ...
  for(auto& danger:dangers_){
    ...
    for(auto& agent:agents_)if(agent.alive){
      ...
      const int distance=world_.distance(agent.position,danger.position);
...
std::string Simulation::execute(Agent&a,const Decision&d){
  ...
  if(d.action=="upgrade_camp_chest"){const auto* definition=ActionRegistry::defaults().action(d.action);const auto fire=world_.nearby_campfire(a.position);if(!a.alive||!definition||definition->operation!="upgrade_camp_chest"||!fire||!world_.upgrade_camp_chest(*fire))return "agrandissement du coffre impossible";add_skill_experience(a,Skill::Building);a.remember("J'ai agrandi le coffre du foyer.");return "agrandit le coffre du camp";}
  ...
  if(d.action=="work_on_building"){
    ...
    const auto infrastructure=before?base_infrastructure(before->type):nullptr;
    if(!a.alive||!a.equipped_tool||a.equipped_tool->type!=CraftItem::Axe||a.equipped_tool->durability<=0||
       ...
       (infrastructure&&!group_.can_evolve(infrastructure->type))||
    ...
    if(after->complete&&infrastructure&&!group_.evolve(infrastructure->type))
      throw std::logic_error("completed infrastructure cannot update the group camp state");
  ...
  if(d.action=="share_map_knowledge"){
    const auto target_id=d.parameters.value("target_id","");
    const auto opportunities=map_sharing_opportunities(a,world_,agents_);
    const auto opportunity=std::find_if(opportunities.begin(),opportunities.end(),[&](const json& candidate){
      return candidate.value("target_id","")==target_id;
    });
    if(opportunity==opportunities.end())return "transmission cartographique indisponible";
    auto target=std::find_if(agents_.begin(),agents_.end(),[&](const Agent& candidate){
      return candidate.id==target_id;
    });
    if(target==agents_.end())return "transmission cartographique indisponible";

    auto transferred=target->map_memory;
    for(const auto& [position,terrain]:a.map_memory)transferred.emplace(position,terrain);
    const auto cells=opportunity->at("cell_count").get<std::size_t>();
    if(transferred.size()!=target->map_memory.size()+cells)return "transmission cartographique indisponible";
    target->map_memory=std::move(transferred);
    a.remember("J'ai partagé ma carte connue avec "+target->name+".");
    target->remember(a.name+" m'a transmis des cases cartographiées.");
    return "transmet "+std::to_string(cells)+" case"+(cells>1?"s":"")+
           " cartographique"+(cells>1?"s":"")+" a "+target->name;
  }
  ...
  if(d.action=="move"){auto dir=d.parameters["direction"].get<std::string>();const auto p=world_.step(a.position,dir);if(p&&world_.passable(*p)){a.position=*p;a.fatigue=clamp_stat(a.fatigue+std::max(0,(50-a.attributes.agility)/20));a.remember_map(*p,world_.terrain(*p));++a.map_visit_counts[{p->x,p->y}];a.remember("Je me suis deplace.");return "se deplace";}if(p)a.remember_map(*p,world_.terrain(*p));a.remember("Mon deplacement a ete bloque par un obstacle.");return "deplacement bloque";}

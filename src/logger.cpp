...
void Logger::ai_feature_request(int cycle,const Agent& agent,const json& report,const json& request) {
  std::string error;
  if(!validate_feature_request(request,error)) { message("Demande IA rejetee : "+error); return; }
  json pending={{"id","cycle-"+std::to_string(cycle)+"-"+agent.id+"-ai-"+std::to_string(++request_counter_)},{"status","pending"},{"source","ai_cycle_report"},{"cycle",cycle},{"agent_id",agent.id},{"agent_name",agent.name},{"report",report},{"title",request["title"]},{"need",request["need"]},{"obstacle",request["obstacle"]},{"proposed_change",request["proposed_change"]},{"mechanism",request["mechanism"]},{"acceptance_tests",request["acceptance_tests"]}};

...
std::vector<std::string> available_actions(const Agent& a, const World& w, const std::vector<Agent>& agents) {
  ...
  if (w.rabbit_alive() && adjacent(a.position,w.rabbit())) r.push_back("hunt_rabbit");
...
bool validate_decision(const Decision& d,const Agent& a,const World& w,const std::vector<Agent>& agents,std::string& e) {
  ...
  static const std::set<std::string> names{"observe","move","wait","talk","sleep","rest","eat_berries","hunt_rabbit"}; if (!names.contains(d.action)) {e="unknown action";return false;}
  ...
  if (d.action=="talk") { if (!d.parameters.is_object() || !d.parameters.contains("target_agent_id") || !d.parameters["target_agent_id"].is_string()) {e="target_agent_id required";return false;} const auto target_id=d.parameters["target_agent_id"].get<std::string>(); bool ok=false; for(const auto&o:agents) if(o.id==target_id && o.alive && adjacent(a.position,o.position)) ok=true; if(!ok){e="talk target not adjacent";return false;} }
  return true;
}

...
namespace apo {
namespace {
bool valid_terrain(Terrain terrain) {
  switch (terrain) {
    case Terrain::Ground:
    case Terrain::Wall:
    case Terrain::Water:
...
bool valid_map_memory(const Agent& agent, const World& world) {
  if (agent.map_memory.size() > static_cast<std::size_t>(World::width * World::height)) return false;
  return std::all_of(agent.map_memory.begin(), agent.map_memory.end(), [&](const auto& entry) {
    return world.in_bounds({entry.first.first, entry.first.second}) && valid_terrain(entry.second);
  });
}

std::size_t transferable_map_cells(const Agent& source, const Agent& target, const World& world) {
  if (!source.alive || !target.alive || !source.home_camp || !target.home_camp ||
      source.home_camp != target.home_camp || !valid_map_memory(source, world) ||
      !valid_map_memory(target, world))
    return 0;
  const auto fire = world.nearby_campfire(source.position);
  if (!fire || *fire != *source.home_camp || world.nearby_campfire(target.position) != fire) return 0;

  std::size_t transferred{};
  for (const auto& [position, terrain] : source.map_memory) {
    (void)terrain;
    if (!target.map_memory.contains(position)) ++transferred;
  }
  return target.map_memory.size() + transferred <=
...
json map_sharing_opportunities(const Agent& source, const World& world,
                               const std::vector<Agent>& agents) {
  json opportunities = json::array();
  for (const auto& target : agents) {
    if (target.id == source.id) continue;
    const auto cells = transferable_map_cells(source, target, world);
    if (cells > 0)
      opportunities.push_back({{"target_id", target.id}, {"target_name", target.name},
                               {"cell_count", cells}});
  }
...
std::vector<std::string> available_actions(const Agent& a, const World& w,
  ...
  if(a.alive&&occupies_current_cell&&a.carried_food){const auto fire=w.nearby_campfire(a.position);if(fire&&w.can_store_food(*fire,*a.carried_food))r.push_back("deposit_food");}
  if(a.alive&&occupies_current_cell&&(a.wood_inventory>0||a.branch_inventory>0||a.iron_ore_inventory>0)){const auto fire=w.nearby_campfire(a.position);if(fire&&w.can_store_materials(*fire,a.wood_inventory,a.branch_inventory,a.iron_ore_inventory))r.push_back("deposit_materials");}
  ...
  if(a.alive&&occupies_current_cell){const auto fire=w.nearby_campfire(a.position);const auto* definition=ActionRegistry::defaults().action("upgrade_camp_chest");if(fire&&definition&&definition->operation=="upgrade_camp_chest"&&w.can_upgrade_camp_chest(*fire))r.push_back("upgrade_camp_chest");}
  ...
  if (!map_sharing_opportunities(a, w, agents).empty()) r.push_back("share_map_knowledge");
...
bool validate_decision(const Decision& d,const Agent& a,const World& w,const std::vector<Agent>& agents,std::string& e,int current_day,DayPhase phase) {
  ...
  if(d.action=="craft_camp_item"){const auto* definition=ActionRegistry::defaults().action(d.action);if(!definition||definition->operation!="craft_recipe"){e="action capability unavailable";return false;}if(!d.parameters.is_object()||!d.parameters.contains("recipe")||!d.parameters["recipe"].is_string()){e="recipe required";return false;}const auto fire=w.nearby_campfire(a.position);if(!fire){e="campfire required";return false;}const auto recipes=w.craftable_recipes(*fire);if(std::find(recipes.begin(),recipes.end(),d.parameters["recipe"].get<std::string>())==recipes.end()){e="recipe unavailable";return false;}}
  if(d.action=="upgrade_camp_chest"){const auto* definition=ActionRegistry::defaults().action(d.action);const auto fire=w.nearby_campfire(a.position);if(!definition||definition->operation!="upgrade_camp_chest"||!fire||!w.can_upgrade_camp_chest(*fire)){e="camp chest upgrade unavailable";return false;}}
  ...
  if(d.action=="share_map_knowledge"){
    if(!d.parameters.is_object()||!d.parameters.contains("target_id")||!d.parameters["target_id"].is_string()){
      e="map sharing target required";return false;
    }
    const auto target_id=d.parameters["target_id"].get<std::string>();
    const auto target=std::find_if(agents.begin(),agents.end(),[&](const Agent& agent){return agent.id==target_id;});
    if(target==agents.end()||transferable_map_cells(a,*target,w)==0){e="map sharing unavailable";return false;}
  }

...
pending_id="$(python3 - "$DATA_DIR/feature_requests.jsonl" "$DATA_DIR/evolution_runs" "$DATA_DIR/approved_feature_requests.jsonl" <<'PY'
import json
import pathlib
import sys
...
for line in requests_path.read_text(encoding="utf-8").splitlines():
    ...
    if request.get("status") == "pending" and request_id not in approved and not (runs_dir / request_id / "validation-record.json").exists():
        print(request_id)
        break
PY
)"
